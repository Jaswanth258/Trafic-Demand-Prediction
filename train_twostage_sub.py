...
print("="*60)
print("Loading train, test, and T2 submissions...")
print("="*60)
...
print("Decoding geohashes and mapping categoricals...")
...
# Map target encodings to Day 48 (OOF for training Stage 1)
print("Computing OOF target encodings for Stage 1 training...")
...
# Fit Stage 2 Ridge model on Day 49 train
print("Fitting Stage 2 Ridge regression model...")
...
# Blending with T2
print("Blending Two-Stage predictions with T2 (v3 GBDT) predictions...")
final_preds = 0.5 * pred_twostage + 0.5 * t2_sub['demand'].values
final_preds = np.clip(final_preds, 0, 1)

# Generate T3 Submission File
sub = pd.DataFrame({
    "Index": test['Index'].values,
    "demand": final_preds
})

sub_path = os.path.join(OUTPUT_DIR, "T3_submission.csv")
