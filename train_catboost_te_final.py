...
print("RUNNING ULTIMATE CATBOOST WITH TARGET ENCODINGS FOR T10 SUBMISSION...")
...
# ==========================================
# 3. TARGET ENCODINGS FROM DAY 48
# ==========================================
print("Computing and mapping target encodings...")

def add_smoothed_te(df_train, df_to_map, group_cols, col_name, smoothing=15):
    agg = df_train.groupby(group_cols)['demand'].agg(['mean', 'count'])
    smoothed = (agg['count'] * agg['mean'] + smoothing * global_mean_48) / (agg['count'] + smoothing)
    smoothed = smoothed.rename(col_name).reset_index()
    mapped_df = df_to_map.merge(smoothed, on=group_cols, how='left')
    return mapped_df[col_name].fillna(global_mean_48).values

# Map target encodings to train and test
train['te_geo'] = add_smoothed_te(train_48, train, 'geohash', 'te_geo', smoothing=20)
test['te_geo'] = add_smoothed_te(train_48, test, 'geohash', 'te_geo', smoothing=20)

train['te_geo_hour'] = add_smoothed_te(train_48, train, ['geohash', 'Hour'], 'te_geo_hour', smoothing=15)
test['te_geo_hour'] = add_smoothed_te(train_48, test, ['geohash', 'Hour'], 'te_geo_hour', smoothing=15)

train['te_geo_ts'] = add_smoothed_te(train_48, train, ['geohash', 'time_slot'], 'te_geo_ts', smoothing=10)
test['te_geo_ts'] = add_smoothed_te(train_48, test, ['geohash', 'time_slot'], 'te_geo_ts', smoothing=10)
...
# ==========================================
# 5. 10-FOLD CATBOOST (Maximum Accuracy)
# ==========================================
...
sub_path = os.path.join(OUTPUT_DIR, 'T10_submission.csv')
...
# ==========================================
# 7. ZIP SOURCE FILES FOR UPLOAD
# ==========================================
print("Zipping source files...")
zip_path = os.path.join(OUTPUT_DIR, 'T10_source_files.zip')
with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
    zipf.write('train_catboost_te_final.py', arcname='train_catboost_te_final.py')
